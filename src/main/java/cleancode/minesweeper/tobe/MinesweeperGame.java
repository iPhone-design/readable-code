...
public class MinesweeperGame {
    ...
    public static final String CLOSED_SELL_SIGN = "□";
    ...
    private static boolean isAllCellOpened() {
        return Arrays.stream(BOARD)             // Stream<String[]>
                ...
                .noneMatch(CLOSED_SELL_SIGN::equals);
    }

    private static int converRowFrom(char cellInputRow) {
    ...
    private static void initializeGame() {
        for (int row = 0; row < BOARD_ROW_SIZE; row++) {
            for (int col = 0; col < BOARD_COL_SIZE; col++) {
                BOARD[row][col] = CLOSED_SELL_SIGN;
            }
        }
    ...
    private static void open(int row, int col) {
        ...
        if (!BOARD[row][col].equals(CLOSED_SELL_SIGN)) {
            return;
        }
        if (isLandMindCell(row, col)) {
            return;
        ...
        if (NEARBY_LAND_MINE_COUNTS[row][col] != 0) {
            BOARD[row][col] = java.lang.String.valueOf(NEARBY_LAND_MINE_COUNTS[row][col]);
            return;
        } else {
